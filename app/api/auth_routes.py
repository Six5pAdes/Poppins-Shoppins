...
def oauth_login():
    ...
    Additionally, the values we passed in to the prompt property will guarantee that we see the screen to select our google account
    ...
    # We will also extract the Referrer Header from the original request sent by our client so that we can send them back to the page they were on when they initiated authentication with Google.
    referrer = request.headers.get('Referrer') # Contains the URL from which the request came.
